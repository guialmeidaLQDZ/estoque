...
function uploadEExportarPDF(html, nomeEvento) {
  ...
  // A partir daqui o TEMP existe — garantir deleção mesmo em caso de erro
  try {
    ...
    // Sempre deleta o TEMP, independente de sucesso ou erro
    try {
      UrlFetchApp.fetch("https://www.googleapis.com/drive/v3/files/" + docTemp.id, {
        method: "DELETE", headers: { "Authorization": "Bearer " + token }, muteHttpExceptions: true
      });
    } catch (e) {
      console.warn("⚠️ Falha ao deletar TEMP doc (" + docTemp.id + "): " + e.message);
    }
  }
}
