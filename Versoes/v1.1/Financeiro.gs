/**
 * ============================================================
 * LIQUIDZ — FINANCEIRO.gs — Projeto v1.1
...
function processarFinanceiroEClickUp() {
  ...
  for (const nome of nomesEventos) {
    ...
    let linkClickUp = "N/A";
    if (listaNF.length > 0) linkClickUp = _criarTarefaClickUp(token, nome, listaNF, custoTotalMateriais);
    ...
    if (sucesso) {
      ...
      totalSucesso++;
      console.log("✅ " + nome + " | NF: " + precisaNF + " | Custo: R$ " + custoTotalMateriais.toFixed(2));
    } else {
      totalErro++;
...
function _criarTarefaClickUp(token, evento, listaNF, custoTotal) {
  ...
  const corpo = {
    ...
    description:
      ...
      "📦 Itens que exigem NF:\n" + listaNF.join("\n"),
    status: "to do", priority: 2, assignees: [FERNANDA_ID_FIN]
  };
