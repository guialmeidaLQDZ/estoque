/**
 * ============================================================
 * LIQUIDZ — SETUPTESTE.gs — Projeto v1.6
 ...
 *   - apagarAbasObsoletas()           → v1.6: remove BASE_BI_LIQUIDZ e ERROS_SISTEMA
 */
...
// ── v1.6: Apaga abas obsoletas da Master ─────────────────────────────────────
function apagarAbasObsoletas() {
  const ss      = SpreadsheetApp.openById(PLANILHAS_PROD.MASTER);
  const obsoletas = ["BASE_BI_LIQUIDZ", "ERROS_SISTEMA"];

  obsoletas.forEach(nome => {
    const sh = ss.getSheetByName(nome);
    if (!sh) { console.warn("⚠️ Aba não encontrada (já apagada?): " + nome); return; }
    ss.deleteSheet(sh);
    console.log("🗑️ Aba apagada: " + nome);
  });
...
function diagnosticarAbasMaster() {
  ...
  const abasGAS = [
    ...
    "EVENTOS_ATIVOS",
    "LOG_API",
    "ADMIN_USERS"   // controle de aprovadores (AppSheet)
  ];
